...
DELAY_CONFIG = {
    'page_crawl': (5, 7),
    'batch_crawl': (5, 10),
    'detail_initial_load': (3, 5),
    'detail_between_items': (8, 12),
    'homepage_visit': (3, 5),
    'retry_delay': (30, 60),
    'captcha_recheck': 2,
}
...
# ============ FUNCTION MỚI: XỬ LÝ LOGIN ============
def check_and_handle_login(driver, worker_id=0):
    """
    Kiểm tra nếu bị redirect về trang đăng nhập (rate limit)
    Mở browser UI để user đăng nhập manual, sau đó tiếp tục
    """
    try:
        current_url = driver.current_url.lower()
        page_source = driver.page_source.lower()
        
        # Check các dấu hiệu của trang rate limit/đăng nhập
        login_indicators = [
            # Check URL
            'dang-nhap' in current_url,
            'login' in current_url,
            'signin' in current_url,
            'authentication' in current_url,
            # Check trang rate limit của alonhadat (CHÍNH XÁC NHẤT)
            'máy của bạn đã truy cập quá nhiều' in page_source,
            'thông báo' in page_source and 'truy cập quá nhiều' in page_source,
            'đăng nhập để thử lại' in page_source,
            # Check form login
            'type="password"' in page_source and 'đăng nhập' in page_source,
            'login-form' in page_source,
            'sign in' in page_source and 'password' in page_source,
        ]
        
        if any(login_indicators):
            print(f"\n[Worker {worker_id}] " + "🔒"*30)
            print(f"[Worker {worker_id}] ⚠️  RATE LIMITED - LOGIN REQUIRED!")
            ...
            # Đóng browser headless
            print(f"[Worker {worker_id}] 🌐 Opening browser with UI for manual login...")
            driver.quit()
            ...
            print(f"[Worker {worker_id}] " + "="*60)
            print(f"[Worker {worker_id}] 👉 PLEASE LOGIN MANUALLY IN THE BROWSER")
            print(f"[Worker {worker_id}] 👉 After login, the script will continue automatically")
            print(f"[Worker {worker_id}] " + "="*60)
            
            # Đợi user đăng nhập - check mỗi 2 giây
            while True:
                time.sleep(2)
                current_url = driver.current_url.lower()
                page_source = driver.page_source.lower()
                
                # Check nếu đã thoát khỏi trang login/rate limit
                still_on_login = any([
                    'dang-nhap' in current_url,
                    'login' in current_url,
                    'signin' in current_url,
                    # Check trang rate limit
                    'máy của bạn đã truy cập quá nhiều' in page_source,
                    'đăng nhập để thử lại' in page_source,
                    # Check form login
                    ('type="password"' in page_source and 'đăng nhập' in page_source)
                ...
                if not still_on_login:
                    print(f"[Worker {worker_id}] ✅ Login successful! Continuing...")
                    time.sleep(3)
                    break
                    
                # Print dots để show đang đợi
                print(f"[Worker {worker_id}] ⏳ Waiting for login...", end='\r')
        ...
        print(f"[Worker {worker_id}] ❌ Error checking login: {e}")
        return driver
    ...
    for row in rows:
        ...
        for i in range(0, len(cols), 2):
            if i + 1 < len(cols):
                ...
                # Check nếu có img tag (icon check)
...
def get_detail_with_selenium(driver, url, worker_id=0, retry_count=0):
    ...
    try:
        ...
        # Check LOGIN sau (nếu bị rate limit) - DÒNG MỚI
        driver = check_and_handle_login(driver, worker_id)
...
def worker_crawl(links_chunk, worker_id, start_headless=True, batch_size=20):
    ...
    try:
        ...
        # Check CAPTCHA và LOGIN khi vào homepage - CẢI TIẾN
        driver = check_and_handle_captcha(driver, worker_id)
        driver = check_and_handle_login(driver, worker_id)
        ...
        for i, link in enumerate(links_chunk, 1):
            ...
            # Delay giữa các item
            if i < len(links_chunk):
                delay = random.uniform(*DELAY_CONFIG['detail_between_items'])
                print(f"[Worker {worker_id}] ⏳ Waiting {delay:.1f}s before next item...")
                time.sleep(delay)
            
            # BATCH DELAY: Sau mỗi batch_size items thì delay 1 phút
            if i % batch_size == 0 and i < len(links_chunk):
                batch_delay = random.uniform(60, 90)
                print(f"\n[Worker {worker_id}] {'🛑'*25}")
                print(f"[Worker {worker_id}] 🎯 BATCH CHECKPOINT: Completed {i}/{len(links_chunk)} items")
                print(f"[Worker {worker_id}] 💤 Taking a break for {batch_delay:.0f}s to avoid rate limit...")
                print(f"[Worker {worker_id}] {'🛑'*25}\n")
...
def crawl_with_selenium_parallel(links, max_workers=2, limit=None, start_headless=True, batch_size=20):
    print(f"\nStarting PARALLEL Selenium crawl")
    print(f"Workers: {max_workers}")
    print(f"Batch size: {batch_size} items (pause 60-90s after each batch)")
    print("="*70)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(worker_crawl, chunk, i+1, start_headless, batch_size): i+1
            for i, chunk in enumerate(chunks)
        }
...
if __name__ == "__main__":
    ...
    if all_links:
        crawl_with_selenium_parallel(
            ...
            limit=len(all_links),             
            start_headless=True,
            batch_size=20  # Pause sau mỗi 20 items
        )
